		    ...
		    for (int row = 0; row < pixels.length-1; row++)
		    {
		           ...
		           col < pixels[0].length-1; col++)
		      {
		        ...
		        if ((leftPixel.colorDistance(rightColor) > 
		            edgeDist) || (leftPixel.colorDistance(bottomColor) > edgeDist) || (leftPixel.colorDistance(brColor) > edgeDist))
		          leftPixel.setColor(Color.BLACK);
		        else
		          leftPixel.setColor(Color.WHITE);
		      }
		...
		for(Pixel[] row : currentPicture)
			{
				for(Pixel currentPixel : row)
					{
						int red = ((int) (Math.random() * 10) + currentPixel.getRed()) % 256;
						int blue = ((int) (Math.random() * 10) + currentPixel.getBlue()) % 256;
						int green = ((int) (Math.random() * 10) + currentPixel.getGreen()) % 256;
